...
func init() {
	collectionsDeleteCollectionV2Cmd.Flags().StringVar(&collectionsDeleteCollectionV2Flags.xOrganizationId, "x-organization-id", "", "The organization ID to scope the request")
	collectionsDeleteCollectionV2Cmd.MarkFlagRequired("x-organization-id")
...
func runCollectionsDeleteCollectionV2(cmd *cobra.Command, args []string) error {
	...
	if cmd.Flags().Changed("x-organization-id") {
		req.Headers["X-Organization-ID"] = fmt.Sprintf("%v", collectionsDeleteCollectionV2Flags.xOrganizationId)
	}
