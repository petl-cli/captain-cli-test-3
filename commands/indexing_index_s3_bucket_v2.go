...
var indexingIndexS3BucketV2Cmd = &cobra.Command{
	Use:   "index-s3-bucket-v2",
	Short: "Index S3 Bucket",
	RunE:  runIndexingIndexS3BucketV2,
}

var indexingIndexS3BucketV2Flags struct {
...
func init() {
	indexingIndexS3BucketV2Cmd.Flags().StringVar(&indexingIndexS3BucketV2Flags.xOrganizationId, "x-organization-id", "", "The organization ID to scope the request")
	indexingIndexS3BucketV2Cmd.MarkFlagRequired("x-organization-id")
	indexingIndexS3BucketV2Cmd.Flags().StringVar(&indexingIndexS3BucketV2Flags.collectionName, "collection-name", "", "Name of the collection to index into")
...
func runIndexingIndexS3BucketV2(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		schema := map[string]any{
			"command":     "index-s3-bucket-v2",
			"description": "Index S3 Bucket",
			"http": map[string]any{
				"method": "POST",
				"path":   "/v2/collections/{collection_name}/index/s3",
			},
	...
	pathParams["collection_name"] = fmt.Sprintf("%v", indexingIndexS3BucketV2Flags.collectionName)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/v2/collections/{collection_name}/index/s3", pathParams),
	...
	if cmd.Flags().Changed("x-organization-id") {
		req.Headers["X-Organization-ID"] = fmt.Sprintf("%v", indexingIndexS3BucketV2Flags.xOrganizationId)
	}
	if cmd.Flags().Changed("idempotency-key") {
		req.Headers["Idempotency-Key"] = fmt.Sprintf("%v", indexingIndexS3BucketV2Flags.idempotencyKey)
	}

	// Request body
	bodyMap := map[string]any{}
	if indexingIndexS3BucketV2Flags.body != "" {
		if err := json.Unmarshal([]byte(indexingIndexS3BucketV2Flags.body), &bodyMap); err != nil {
	...
	if cmd.Flags().Changed("bucket-name") {
		bodyMap["bucket_name"] = indexingIndexS3BucketV2Flags.bucketName
	}
	if cmd.Flags().Changed("bucket-region") {
		bodyMap["bucket_region"] = indexingIndexS3BucketV2Flags.bucketRegion
	}
	if cmd.Flags().Changed("aws-access-key-id") {
		bodyMap["aws_access_key_id"] = indexingIndexS3BucketV2Flags.awsAccessKeyId
	}
	if cmd.Flags().Changed("aws-secret-access-key") {
		bodyMap["aws_secret_access_key"] = indexingIndexS3BucketV2Flags.awsSecretAccessKey
	}
	if cmd.Flags().Changed("processing-type") {
		bodyMap["processing_type"] = indexingIndexS3BucketV2Flags.processingType
	}
	if cmd.Flags().Changed("max-files") {
		bodyMap["max_files"] = indexingIndexS3BucketV2Flags.maxFiles
	}
	if cmd.Flags().Changed("skip-existing") {
		bodyMap["skip_existing"] = indexingIndexS3BucketV2Flags.skipExisting
	}
	if cmd.Flags().Changed("parsing-script") {
		bodyMap["parsing_script"] = indexingIndexS3BucketV2Flags.parsingScript
	}
	req.Body = bodyMap
