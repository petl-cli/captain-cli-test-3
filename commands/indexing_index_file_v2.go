...
var indexingIndexFileV2Cmd = &cobra.Command{
	Use:   "index-file-v2",
	Short: "Index File Upload",
	RunE:  runIndexingIndexFileV2,
}

var indexingIndexFileV2Flags struct {
...
func init() {
	indexingIndexFileV2Cmd.Flags().StringVar(&indexingIndexFileV2Flags.xOrganizationId, "x-organization-id", "", "The organization ID to scope the request")
	indexingIndexFileV2Cmd.MarkFlagRequired("x-organization-id")
	indexingIndexFileV2Cmd.Flags().StringVar(&indexingIndexFileV2Flags.collectionName, "collection-name", "", "Name of the collection to index into")
...
func runIndexingIndexFileV2(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		flags = append(flags, flagSchema{
			...
			Required:    false,
			Location:    "body",
			Description: "Document processing type: 'advanced' for AI-enhanced extraction, 'basic' for standard processing",
		})
		flags = append(flags, flagSchema{
			...
			Required:    false,
			Location:    "body",
			Description: "JSON string of custom metadata to attach to all indexed chunks",
		})
		...
		schema := map[string]any{
			"command":     "index-file-v2",
			"description": "Index File Upload",
			"http": map[string]any{
				"method": "POST",
	...
	pathParams["collection_name"] = fmt.Sprintf("%v", indexingIndexFileV2Flags.collectionName)

	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/v2/collections/{collection_name}/index/file", pathParams),
	...
	if cmd.Flags().Changed("x-organization-id") {
		req.Headers["X-Organization-ID"] = fmt.Sprintf("%v", indexingIndexFileV2Flags.xOrganizationId)
	}
