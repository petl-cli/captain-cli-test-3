...
func init() {
	...
	companiesCmd.AddCommand(companiesSearchCmd)
}

func runCompaniesSearch(cmd *cobra.Command, args []string) error {
	...
	if rootFlags.schema {
		...
		flags = append(flags, flagSchema{
			...
			Required:    true,
			Location:    "query",
			Description: "Company name, domain, or natural language query (e.g. 'AI startups in San Francisco raising Series B')",
		})
		flags = append(flags, flagSchema{
			Name:        "limit",
			Type:        "integer",
	...
	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/v2/datasets/odyssey/companies/search", pathParams),
	...
	if cmd.Flags().Changed("q") {
		req.QueryParams["q"] = fmt.Sprintf("%v", companiesSearchFlags.q)
	}
	if cmd.Flags().Changed("limit") {
		req.QueryParams["limit"] = fmt.Sprintf("%v", companiesSearchFlags.limit)
	}
